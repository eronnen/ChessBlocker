...
export async function setDailyLimitMessages(website: ChessWebsiteType, gamesPlayedElement: Element, gamesAllowedElement: Element): Promise<void> {
    ...
    gamesAllowedElement.innerHTML = `${items[website + ".gamesPerDay" as GamesPerDayConfigType]![currentWeekday]} games were allowed today (${WEEK_DAYS[currentWeekday]})`;
    ...
    const gamesPlayedToday = items[website + ".gamesPlayedToday" as GamesPlayedTodayConfigType]!;
    if (gamesPlayedToday >= 0) {
        ...
        try {
            ...
            const gamesCount = await getWebsiteGamesCount(website, username, dayStart);
