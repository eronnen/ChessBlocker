...
export function addLoadingAnimation(): () => void {
  ...
  animationStyle.innerHTML =
    "@keyframes ChessBlockerLoading { 0% { opacity: 1; transform: translateY(3.5rem); } 100% { opacity: 1; transform: translateY(0); } }";
  document.head.appendChild(animationStyle);

  const loadingElement = document.createElement("div");
  loadingElement.innerHTML = `ChessBlocker: checking games...`;
  loadingElement.style.cssText =
    "position: fixed; bottom: 0; left: 40%; font-size: 21px; font-weight: bold; color: black; background: rgb(190, 190, 190); border: 0.15em solid rgb(77, 77, 77); border-radius: 5px;";
  loadingElement.style.setProperty(
    "animation",
    "ChessBlockerLoading 0.5s ease-out 1s backwards",
  );
...
export async function playButtonHandler(
  event: ChessBlockerEvent,
  website: ChessWebsiteType,
  is_new_game_link: boolean,
  getPreviousGamesTimesPromise: (
  ...
  try {
    ...
    previousGamesTimes =
      await getPreviousGamesTimesPromise(chessBlockerOptions);
  } catch (err) {
  ...
  const gamesLimitPerToday =
    chessBlockerOptions[(website + ".gamesPerDay") as GamesPerDayConfigType]![
      getActualWeekDayByDate(
        currentDate,
        chessBlockerOptions.dayStartTimeHours!,
        chessBlockerOptions.dayStartTimeMinutes!,
      )
    ];

  const numberOfGamesPlayed = previousGamesTimes.filter(
    (t) => t > dayStartEpochMillis,
  ).length;
  if (numberOfGamesPlayed >= gamesLimitPerToday) {
    // limit reached
    console.debug(
      `you reached your daily games limit! you played ${numberOfGamesPlayed}/${gamesLimitPerToday} games`,
    );
    const setConfig: ChessBlockerConfigType = {
      [(website + ".gamesPlayedToday") as GamesPlayedTodayConfigType]:
        numberOfGamesPlayed,
    };
    ...
    console.debug(
      `you played ${numberOfGamesPlayed}/${gamesLimitPerToday} games today`,
    );
    reclickButton();
  }
}

export function addPlayButtonHandlerWithPattern(
  parent: HTMLElement,
  tag: string,
  buttonPattern: RegExp,
  handler: (event: ChessBlockerEvent) => void,
) {
  for (const e of parent.querySelectorAll(tag)) {
    if (
      e instanceof Element &&
      e.textContent &&
      e.textContent.match(buttonPattern)
    ) {
