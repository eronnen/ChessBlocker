export async function* readLichessGamesResponseLines(
  response: Response,
  ...
  while (true) {
    ...
    if (chunk) {
      ...
      while (currentIndex < chunk.length) {
        ...
        yield utf8Decoder.decode(
          chunk.slice(currentIndex, nextLineEndingIndex),
        );
...
export async function getLichessGamesCount(
  username: string,
  fromDate: Date,
  maxGames: number = 0,
) {
  return getLichessGames(username, fromDate, maxGames).then(
    async (response) => {
      let numberOfGames = 0;
...
export function getLichessGames(
  username: string,
  fromDate: Date,
  maxGames = 0,
) {
