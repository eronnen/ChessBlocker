...
async function waitForSideBarAndAddListener(): Promise<HTMLElement> {
    const sideBarElement = await waitForElementToExist("board-layout-sidebar") as HTMLElement;
    if (!sideBarElement) {
...
async function initializeChessBlocker() {
    ...
    if (pagePath == '/home') {
        ...
        if (!addPlayButtonHandlerWithPattern(document.body, "a", /^\s*Play (\d+) min\s*$/, (event) => {
        ...
        reinitializeChessBlockerData();
        const sideBarElement = await waitForSideBarAndAddListener() as HTMLElement;

        const sidebarObserver = new MutationObserver((mutationList: MutationRecord[]) => {
